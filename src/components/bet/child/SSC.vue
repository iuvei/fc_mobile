<template>
	<div class="ssc">
		<div class="clockView">
			<div class="left">
				<p style="display: flex;align-items: center;text-align: center;justify-content: center;"><img src="../../../../static/img/lingdang.png" style="width: 0.3rem;margin-right:0.1rem;" />{{gameConfig[kindId].gameName}}第{{kindId!='azssc'?showPrevAwarPeriod:prevAwarPeriod2}}期</p>
				<ul>
					<li class="circle" v-for="(item,index) in awardNum" style="overflow: hidden;" :key="index">
						...
						<!--<ul style="height: 4.5rem;width: 0.9rem;display: block;" class="active_ul" v-else>
							<li class="num" v-for="item in awardNum" >
								{{item}}
							</li>
						</ul>-->
					</li>
			...
			<div class="right">
				...
				<CountDown class="clock-count" :format="':'" :classId="classId" :kindId="kindId" style="font-size: 14px;color:#e43a45;" @stop="countDown" :historyData="historyData"></CountDown>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState("bet", ["subGameId", "leftName", "rightName", "gameId", "gamePlayList","kindList","historyData"]),
